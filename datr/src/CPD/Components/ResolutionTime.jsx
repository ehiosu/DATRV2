            ...
            <motion.section initial={{ scale: 0.1, opacity: 0 }} exit={{ scale: 0.1, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-[90%] space-y-8 mx-auto max-h-[80vh] md:overflow-y-auto">
                ...
                <div className="w-full  flex lg:flex-row flex-col p-2 lg:gap-0 gap-4  bg-white rounded-md shadow-lg">
                    <div className="flex-1  ">
                        <DonutChart colors={['#00CCF2']} data={[65]} labels={['First Reply Solved']}/>
                    </div>
                    <div className="flex-1">
                    <DonutChart colors={['#1BB99D']} data={[80]} labels={['Assigned Tickets']}/>
                    </div>
                </div>
...
const GridCard = ({ title, text, trend, percentage }) => {
    return (
        <div className="w-[90%] md:h-[25vh] h-[20vh] rounded-lg bg-white flex flex-col shadow-md   p-2 text-center">
            <PiWarningCircle className='ml-auto  text-[1.2rem]   hover:cursor-pointer' />
            <p className="text-center lg:text-[1.4rem] md:text-[0.975rem] text-[1rem] font-semibold">{title}</p>
            <p className="font-semibold text-[1.4rem] md:text-[2.1rem] mt-4">{text}</p>
            <p className={`${trend === "up" ? 'text-blue-400' : 'text-red-500'} mt-auto`}>{percentage}</p>
        </div>
    )
}
...
const DonutChart = ({data,labels,title,colors}) => {
  ...
  return (
    <div    className='w-full md:w-[80%] h-[35vh]  p-2 ' >
        <ReactApexChart height={'95%'} options={options} series={series} type='donut' />
    </div>
