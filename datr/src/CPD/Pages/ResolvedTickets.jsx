...
import { ResolvedTicketsTable } from "../Components/DataTable";
...
export const ResolvedTickets = () => {
  ...
  const ticketsQuery = useQuery({
    ...
    queryFn: () =>
      axios("tickets/status?value=RESOLVED", {
        method: "GET",
      })
        .then((resp) => resp.data)
        .catch((err) => err),
  ...
  return (
    <section className="w-full">
      <SearchPage heading={"Resolved Tickets"}>
        ...
        <div className="w-full    max-h-[75vh]  mt-4 overflow-y-auto ">
          {ticketsQuery.isSuccess && (
            <ResolvedTicketsTable data={ticketsQuery.data.tickets} />
          )}
