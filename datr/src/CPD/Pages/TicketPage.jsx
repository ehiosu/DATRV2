...
const AddReminderPopover = () => {
  ...
  const reminderMutation = useMutation({
    mutationKey: ["ticket", `${id}`, "reminder"],
    mutationFn: () =>
      new Promise((resolve, reject) =>
        axios(`tickets/airline/reminder/${id}`, {
          method: "GET",
        })
    ...
    onError: (error) => {
      ...
      if (error.response?.data?.message) {
        ...
        setErrorMessage("An error occurred while sending the reminder.");
      }
    },
  });
...
const SendToAirlinePopover = () => {
  ...
  const sendToAirlineMutation = useMutation({
    mutationKey: ["send", `${id}`, "airline"],
    mutationFn: () =>
      new Promise((resolve, reject) =>
        axios(`tickets/airline/action/${id}`, {
          method: "PUT",
        })
          .then((resp) => resolve(resp))
          .catch((err) => reject(err))
    ...
    onSuccess: () => {
      console.log("Sent to Airline successfully.");
      client.invalidateQueries(["tickets", `${id}`]);
    },
    onError: (error) => {
      console.error("Error sending to airline:", error);
      if (error.response?.data?.message) {
        setErrorMessage(error.response.data.message);
      } else {
        setErrorMessage("An error occurred while sending to airline.");
      }
    },
  });
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        ...
        className="w-80"
      >
        <p>Do you want to send to airline?</p>
        {errorMessage && (
          <p className="text-red-500 text-sm my-2">{errorMessage}</p>
        )}
        <button
          className="w-max px-3 py-1.5 bg-ncBlue text-white mt-3"
  ...
  return (
    <Popover>
      ...
      <PopoverContent side="bottom" className="w-44">
        ...
        <AuthorizedComponent roles={["FOU_CPO", "FOU_HEAD", "ADMIN"]}>
          <SendToAirlinePopover />
        </AuthorizedComponent>
...
//                         )}
//                         viewerInfo={viewerInfo[comment.id]}
//                         // viewerInfo={renderViewerInfo(comment)}
//                       />
