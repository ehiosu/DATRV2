...
export const useTickets = (
  ...
  const queryFn = () =>
    axios(queryUrl, { method: "GET" }).then((resp: any) => {
...
const GetRequestUrl = (requestargs: requestParams) => {
  ...
  const isTerminalHeadLike =
    role === "TERMINAL_SUPERVISOR" || role === "SHIFT_SUPERVISOR";
  const isAirline = role === "AIRLINE";

  if (isCPO) {
    return `tickets/by-creator-assignee?email=${email}&page=${
      page - 1
    }&size=${size}`;
  }
  if (isAdmin) {
    ...
    return status === "AWAITING_APPROVAL"
      ? `tickets/terminal/status/department?terminal-name=${terminal}&ticket-status=AWAITING_APPROVAL&page=${
          page - 1
        }&size=${size}`
      : `tickets/valid/status?value=${status}&page=${page - 1}&size${size}`;
  }
  if (isInFOU) {
    if (role === "FOU_HEAD") {
      return `tickets/terminal/status/department?terminal-name=${terminal}&ticket-status=${status}&page=${
        page - 1
      }&size=${size}`;
    }
    return `tickets/by-creator-assignee?email=${email}&page=${
  ...
  if (isAirline) {
    let url =
      status === "ALL"
        ? `tickets/airline?page=${page - 1}&size=${size}`
        : `tickets/airline/status?value=${status}&page=${
            page - 1
          }&size=${size}`;
