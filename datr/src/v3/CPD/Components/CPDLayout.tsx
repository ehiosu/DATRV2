...
export const CPDLayout = () => {
  ...
  if (!isAccessValid && location.pathname!="/CPD/New-Ticket" ) {
    return <Navigate to={"/"} />;}
  if(!verified && !isAccessValid && location.pathname != "/CPD/New-Ticket") {
  ...
  return (
    <div className='flex  w-full  relative overflow-hidden'>
        ...
        {
          isAccessValid && <button onClick={()=>{nav(-1)}} className='w-max text-xs px-3 h-max py-1.5 my-2 bg-ncBlue rounded-md text-white flex items-center'>
