...
import { GenericDataTable, reportEntryColumnDef } from '@/CPD/Components/DataTable'

export const CancelledReports = () => {
    const {id}=useParams()
    const {terminal,date,setDate}=useTerminalStore()
    const query=useTerminalData(terminal,`${id?.replace("_"," ")}`,"cancelled",date as DateRange)
  ...
  return (
    ...
    <div onClick={()=>{nav(-1)}} role='button' className='w-7 h-7 bg-ncBlue  flex items-center justify-center text-white rounded-md'>
        <MdArrowBack className='w-4 h-4 shrink'/>
    </div>
    <div className="flex items-center w-full justify-between">
    <p className='text-xl font-semibold'>Cancelled Flights for {id?.replace("_"," ")}</p>
    ...
    <div className="w-full h-[60vh] overflow-y-auto border-2 border-neutral-300 rounded-lg py-1 mt-4 scroll-smooth">
      <p className="my-2 ml-2 text-lg font-semibold text-ncBlue">
        Arrivals
          ...
          {
            query.isLoading?<Skeleton className='w-full h-[60vh]'/>:query.isSuccess&&   <GenericDataTable tableClassname='' downloadExcel DownloadComponent={ReportDownloadComponent} headerClassname='rounded-lg' columns={reportEntryColumnDef as any} data={splitData.Arrival} hasFilter={true} showColumnFilter filterColumn={"airline"}/>
          }
        </div>
    <div className="w-full h-[60vh] overflow-y-auto border-2 border-neutral-300 rounded-lg py-1 mt-4 scroll-smooth">
      <p className="my-2 ml-2 text-lg font-semibold text-ncBlue">
        Departures
          ...
          {
            query.isLoading?<Skeleton className='w-full h-[60vh]'/>:query.isSuccess&&   <GenericDataTable tableClassname='' downloadExcel DownloadComponent={ReportDownloadComponentDeparture} headerClassname='rounded-lg' columns={reportEntryColumnDef as any} data={splitData.Departure} hasFilter={true} showColumnFilter filterColumn={"airline"}/>
          }
        </div>
    </section>
  )
      ...
      new Promise((resolve, reject) => {
        ...
        Object.keys(filteredData).map((dateKey)=>{
              ...
              workbook.xlsx.writeBuffer().then(function (data) {
                ...
                anchor.href = url;
                anchor.download = `Arrival Cancelled Flights Report for ${airline}${nativeDate.from}-${nativeDate.to}.xlsx`;
                anchor.click();
      ...
      new Promise((resolve, reject) => {
        ...
        Object.keys(filteredData).map((dateKey)=>{
              ...
              workbook.xlsx.writeBuffer().then(function (data) {
                ...
                const anchor = document.createElement("a");
                anchor.href = url;
                anchor.download = `Departure Cancelled Flights Report for ${airline}${nativeDate.from}-${nativeDate.to}.xlsx`;
                anchor.click();
                window.URL.revokeObjectURL(url);
              });
        } catch (err: any) {
